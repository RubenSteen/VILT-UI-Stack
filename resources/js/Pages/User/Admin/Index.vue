                            ...
                            <filters :show-sidebar="showFilterSidebar" :filters="filters" @applyFilters="applyFilters" @reset="resetFilters" @toggleSidebar="toggleFilterSidebar">
                                <div class="px-4 divide-y divide-gray-200 sm:px-6">
                                    <div class="space-y-6 pt-6 pb-5">
                                        ...
                                        <div>
                                            <label for="email_not_verified" class="block text-sm font-medium text-gray-900">
                                                Unverified email
                                            </label>
                                            <div class="mt-1">
                                                <button @click="filtersForm.email_not_verified = !filtersForm.email_not_verified" id="email_not_verified" type="button" aria-pressed="false" :class="{ 'bg-indigo-600 border-indigo-600 focus:ring-indigo-500': filtersForm.email_not_verified, 'bg-red-600 border-red-600 focus:ring-red-500': !filtersForm.email_not_verified,}" class="relative inline-flex flex-shrink-0 h-6 w-11 border-2 rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2">
                                                    <span class="sr-only">Use setting</span>
                                                    <span :class="{ 'translate-x-5': filtersForm.email_not_verified, 'translate-x-0': !filtersForm.email_not_verified,}" class="translate-x-0 relative inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200">
                                                                    <span :class="{ 'opacity-0 ease-out duration-100': filtersForm.email_not_verified, 'opacity-100 ease-in duration-200': !filtersForm.email_not_verified,}" class="opacity-100 ease-in duration-200 absolute inset-0 h-full w-full flex items-center justify-center transition-opacity" aria-hidden="true">
                                                                        <svg class="h-3 w-3 text-gray-400" fill="none" viewBox="0 0 12 12">
                                                                            <path d="M4 8l2-2m0 0l2-2M6 6L4 4m2 2l2 2" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                                                        </svg>
                                                                    </span>
...
export default {
    ...
    watch: {
        ...
        'filtersForm.email_verified': function (newValue, oldValue) {
            if (newValue === true) {
                this.filtersForm.email_not_verified = false;
            }
        },

        'filtersForm.email_not_verified': function (newValue, oldValue) {
            if (newValue === true) {
